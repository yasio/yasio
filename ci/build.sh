...
function build_android()
{
    ...
    cd $YASIO_ROOT/build
    mkdir -p build_armv7
    cmake .. -G "Ninja" -Bbuild_armv7 -DANDROID_STL=c++_shared -DCMAKE_MAKE_PROGRAM=~/ninja -DCMAKE_TOOLCHAIN_FILE=~/android-ndk-$NDK_VER/build/cmake/android.toolchain.cmake -DCMAKE_ANDROID_NDK_TOOLCHAIN_VERSION=clang -DANDROID_ABI=armeabi-v7a -DCMAKE_BUILD_TYPE=Release -DYASIO_SSL_BACKEND=1 -DYASIO_HAVE_CARES=ON
