...
void append_msg(std::vector<char>& blob, std::string_view msg)
{
  ...
  service.start_service(hosts, _ARRAYSIZE(hosts), [&](event_ptr&& event) {
    switch (event->kind())
    {
      case YEK_PACKET: {
        ...
        append_msg(blob, "hello client, my ip is:");
        append_msg(blob, event->transport()->local_endpoint().to_string());
        append_msg(blob, "\n");
        service.write(event->transport(), std::move(blob));
        break;
      }
      case YEK_CONNECT_RESPONSE:
        if (event->status() == 0)
