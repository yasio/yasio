...
#define HWP_PACKET_HEADER_LENGTH 16
#define _DEFINE_KEYS \
const unsigned char information_key[] = { 0xca,0x18,0x1e,0x53,0x4d,0x94,0xfb,0x3e,0x48,0x3f,0x36,0x97,0x34,0x9a,0xcb,0xb4,0x91,0x03,0x03,0x17,0x6e,0x99,0x49,0x89,0xf1,0xec,0x5e,0x6d,0xe7,0x90,0x79,0x2b, };
    ...
    if (ctx_.expected_size_ == -1) { // decode length
        if (decode_pdu_length(ctx_.buffer_, ctx_.offset_ + n, ctx_.expected_size_, &ctx_.header_))
        {
            if (ctx_.expected_size_ < 0) { // header insuff
                ...
                ctx_.receiving_pdu_.reserve(ctx_.expected_size_); // #perfomance, avoid memory reallocte.

                auto bytes_transferred = n + ctx_.offset_;
                ctx_.receiving_pdu_.insert(ctx_.receiving_pdu_.end(), ctx_.buffer_, ctx_.buffer_ + (std::min)(ctx_.expected_size_, bytes_transferred));
        ...
        if ((ctx_.receiving_pdu_.size() + bytes_transferred) > MAX_PDU_LEN) // TODO: config MAX_PDU_LEN, now is 16384
        {
            ...
            auto bytes_needed = ctx_.expected_size_ - static_cast<int>(ctx_.receiving_pdu_.size()); // never equal zero or less than zero
            if (bytes_needed > 0) {
                ctx_.receiving_pdu_.insert(ctx_.receiving_pdu_.end(), ctx_.buffer_, ctx_.buffer_ + (std::min)(bytes_transferred, (bytes_needed)));
...
void connection::send_to_peer(const buffer_type& buffer)
{
    if (!this->peer_.expired())
    {
        ...
        {
            boost::system::error_code ignored_ec;
            peer_self->get_socket().close(ignored_ec);
        }
