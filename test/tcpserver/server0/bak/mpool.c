...
/* version */
//static	const char *version = "mpool library version 2.1.1";
...
/****************************** local utilities ******************************/
#ifdef _WIN32
static u_long getpagesize(void)
{
			     ...
			     const unsigned long size)
{
  ...
  if (bit_n < min_bit_free_next) {
    ...
    if (mp_p->mp_free[bit_n] == NULL) {
      mp_p->mp_free[bit_n] = (struct mpool_block_st*)addr;
    }
  }
			   ...
			   int *error_p)
{
  ...
  if (bit_c > MAX_BITS) {
    ...
    if (bit_c < min_bit_free_next) {
      ...
      mp_p->mp_free[bit_c] = (struct mpool_block_st *)free_pnt.mf_next_p;
